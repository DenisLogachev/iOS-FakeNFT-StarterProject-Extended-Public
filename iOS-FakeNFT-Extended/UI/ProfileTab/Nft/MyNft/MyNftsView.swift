...
struct MyNftsView: View {
    @Environment(ServicesAssembly.self) private var ass
    @Environment(\.dismiss) private var dismiss
    @AppStorage(NFTSortOrder.myNFTStorageKey) private var nftSortOrder: NFTSortOrder = .byInput
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Button(NSLocalizedString("MyNfts.sort.byPrice", comment: "")) { nftSortOrder = .byPrice }
            Button(NSLocalizedString("MyNfts.sort.byRating", comment: "")) { nftSortOrder = .byRating }
            Button(NSLocalizedString("MyNfts.sort.byName", comment: "")) { nftSortOrder = .byName }
    ...
    private var content: some View {
        ...
        case .loaded(let nfts):
            List {
                ForEach(nfts.sorted(by: nftSortOrder)) { nft in
                    MyNftRow(
                        nft: nft,
