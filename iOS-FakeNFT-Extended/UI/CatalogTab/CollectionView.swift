...
struct CollectionView: View {
    ...
    @Environment(CatalogNavRouter.self) private var navRouter
                ...
                LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 9), count: 3), spacing: 28) {
                    ForEach(nftCollection.uniqueNfts, id: \.self) { nftID in
                        if let nft = fetchedNFTs[nftID], let nft {
                            NftView(catalogVM: catalogVM, nft: nft)
...
#Preview {
    CollectionView(
        ...
        nftCollection: NFTCollection.mockCollections.first!,
        coverImage: Image(.peachGroup)
    )
